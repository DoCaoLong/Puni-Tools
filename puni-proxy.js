...
class UniquidAPIClient {
    ...
    loadProxies() {
        try {
            if (fs.existsSync(this.proxyFile)) {
                return fs.readFileSync(this.proxyFile, 'utf8')
                    .replace(/\r/g, '')
                    .split('\n')
                    .filter(Boolean);
            }
    ...
    async makeRequest(method, url, options = {}) {
        const { data, proxyUrl, ...restOptions } = options;
        
        if (proxyUrl) {
            const proxyAgent = new HttpsProxyAgent(proxyUrl);
            restOptions.httpsAgent = proxyAgent;
        }
    ...
    async getUserProfile(userId, token, tp, proxyUrl) {
        if (!token || !tp) {
    ...
    async getTaskList(userId, token, tp, proxyUrl) {
        if (!token || !tp) {
            this.log(`Thiếu thông tin token hoặc tp cho user ${userId}`, 'error');
            return null;
        }
    ...
    async checkTask(type, userId, token, tp, proxyUrl) {
        if (!token || !tp) {
            this.log(`Thiếu thông tin token hoặc tp cho user ${userId}`, 'error');
            return false;
        }
    ...
    async getQuestion(userId, token, tp, proxyUrl) {
        if (!token || !tp) {
            this.log(`Thiếu thông tin token hoặc tp cho user ${userId}`, 'error');
            return null;
        }
    ...
    async submitAnswer(userId, token, tp, answer, proxyUrl) {
        if (!token || !tp) {
            this.log(`Thiếu thông tin token hoặc tp cho user ${userId}`, 'error');
            return null;
        }
    ...
    async handleQuiz(userId, token, tp, chances, proxyUrl) {
        if (chances <= 0) {
        ...
        while (chances > 0) {
            const question = await this.getQuestion(userId, token, tp, proxyUrl);
            if (!question) {
...
//            const randomAnswer = Math.floor(Math.random() * 2);
            ...
            const result = await this.submitAnswer(userId, token, tp, randomAnswer, proxyUrl);
    ...
    async processAllTasks(userId, token, tp, proxyUrl) {
        if (!token || !tp) {
            this.log(`Thiếu thông tin token hoặc tp cho user ${userId}`, 'error');
            return;
        }
        ...
        try {
            const taskList = await this.getTaskList(userId, token, tp, proxyUrl);
            if (!taskList) {
                this.log('Không thể lấy danh sách nhiệm vụ', 'error');
                return;
            }

            const profile = await this.getUserProfile(userId, token, tp, proxyUrl);
            ...
            for (const task of taskList) {
                ...
                const success = await this.checkTask(task.type, userId, token, tp, proxyUrl);
            ...
            const updatedProfile = await this.getUserProfile(userId, token, tp, proxyUrl);
            if (updatedProfile && updatedProfile.data) {
                ...
                if (updatedProfile.data.chances > 0) {
                    await this.handleQuiz(userId, token, tp, updatedProfile.data.chances, proxyUrl);
                }
            }
    ...
    async getUserTaskSummary(userId, token, tp, proxyUrl) {
        const profile = await this.getUserProfile(userId, token, tp, proxyUrl);
    ...
    async main() {
        ...
        while(true) {
            for (let i = 0; i < data.length; i++) {
                ...
                if (needNewToken) {
                    const loginResult = await this.login(initData, proxyUrl);
                ...
                const profile = await this.getUserProfile(userId, currentToken, currentTP, proxyUrl);
                if (profile && profile.data) {
                    const { point, correct, wrong } = profile.data;
                    this.log(`Point: ${point} | Đúng: ${correct} | Sai: ${wrong}`, 'custom');
                }

                await this.processAllTasks(userId, currentToken, currentTP, proxyUrl);
